...
src_unpack() {
	# We use prebuilt sbcl binaries to bootstrap from sources our own sbcl binaries.
	# The real problem starts when the unpacked directory of the downloaded 
	# binaries does not have a regular naming scheme, eg. 
	# sbcl-1.0.19-x86-linux-binary.tar.bz2    --> sbcl-1.0.19-x86-linux
	# sbcl-1.0.19-x86-64-linux-binary.tar.bz2 --> sbcl-1.0.19
	# sbcl-1.0.19-source.tar.bz2              --> sbcl-1.0.19
	# this means that in amd64 machines, when doing unpack ${A}
	# it results in unpacking both the sbcl-1.0.19-source.tar.bz2 and 
	# sbcl-1.0.19-x86-64-linux-binary.tar.bz2 contents in the same 
	# "sbcl-1.0.19" directory. 
	# In order to avoid this, we introduce the code below.
	# If there is a distfile that is NOT ${P}-source.tar.bz2, we unpack
	# it in temp directory, we rename it to sbcl-binary, and we put it back
	# to ${WORKDIR}. Then, we unpack the sane sources which will be under the 
	# directory ${S}. In some cases, users include wrong arch flags in 
	# their USE flag set. Eg. they use x86 on machines keyworded amd64. 
	# This results in 3 distfiles instead of 2, and it's wrong and bad,so we 
	# check this behaviour with the $url_num variable. When this variable is
	# above 2, the ebuild dies.
	local url_num=0
	local bin="sbcl-binary"
	for url in ${A}
	do 
		(( url_num++ )) 	
		if [[ "${url}" != "${P}-source.tar.bz2" ]]
		then
			   mkdir temp \
			&& cd temp \
			&& unpack ${url} \
			...
			&& cd .. \
			&& mv temp/${bin} .  \
			|| die "Unpacking ${P}-source.tar.bz2 and \
binary tarball needed for bootstrap, failed"
		else
			unpack ${url}
		fi
	if [[ "$url_num" -gt 2 ]]
	then
		die "More than two distfiles. Please remove from your USE flags any \
wrong arch flags, eg x86 on an \"amd64 keyworded\" system"
	fi
	done
	cd "${S}"
