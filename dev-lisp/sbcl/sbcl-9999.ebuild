...
RDEPEND="elibc_glibc? ( >=sys-libs/glibc-2.3
...
src_configure() {
	...
	echo -e ";;; Auto-generated by Gentoo\n\"gentoo\"" > branch-version.lisp-expr
...
src_compile() {
	...
	env - HOME="${T}" \
		PATH="${bindir}/src/runtime:${PATH}" \
		SBCL_HOME="${bindir}/output" \
...
EOF
	...
	if use source; then
		find src -type f '(' -name '*.fasl' -or -name '*.d' -or -name '*.o' ')' -delete
		cp -a src "${D}"/usr/$(get_libdir)/sbcl/
	fi
