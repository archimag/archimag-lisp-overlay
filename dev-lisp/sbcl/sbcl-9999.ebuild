...
#	epatch "${FILESDIR}/disable-tests-gentoo-${PV}.patch"
	...
	# Just tag the original version otherwise scm ebuilds would for ever be 99999
	sed -i -e "s|^\"\(.*\)\"$|\"\1-gentoo-${P}\"|g" "${S}/version.lisp-expr" || \
		die "Sed failed on version.lisp-expr."
