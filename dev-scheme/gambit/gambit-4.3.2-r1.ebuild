...
#	sed "s%dstdir=%dstdir=${D}%" -i install-sh
#	diff -u install-sh.old install-sh

#	cp makefile.in makefile.in.old
#	sed "s%\$(prefix)%\$(DESTDIR)\$(prefix)%g" -i makefile.in
#	diff -u makefile.in.old makefile.in

#	epatch "${FILESDIR}"/gambc-v4.2.8.patch

#	cp gsi/makefile.in gsi/makefile.in.old

	find -name tests -prune -o -name makefile.in -print | \
		xargs sed --in-place -f ${FILESDIR}/add_DESTDIR-to_INSTALL.sed

#	diff -u gsi/makefile.in.old gsi/makefile.in
...
#	sed 's:$DASH_O1::' -i configure.ac
	...
	sed -e "s:\(LINK='\$(C_COMPILER)\):\1 -Wl,--rpath -Wl,\$(libdir):" -i configure.ac
...
#	diff -u configure.ac.old configure.ac
	...
	diff -urd ~marijn/gambit/gambc-v4_3_2/ . | tee ../DESTDIR.patch
	eautoreconf
}
